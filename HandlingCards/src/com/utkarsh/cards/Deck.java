...
/**
 * An object of type Deck represents a deck of playing cards. The deck contains
 * 52 regular cards and that can also optionally include two Jokers.
 * 
 * @author Utkarsh
 *
 */
public class Deck {
	 ...
	 */
	public void shuffle() {
		// Starting Reverse and shuffle
		for (int i = deck.length - 1; i > 0; i--) {
			int rand = (int) (Math.random() * (i + 1));
			// Shuffling using simple swap
			Card temp = deck[i];
			deck[i] = deck[rand];
			deck[rand] = temp;
		}
	...
	public Card dealCard() {
		...
		// Cards are not literally removed from the array that represents the
		// deck.
		// We just keep track of how many cards have been used.
	}

	/**
	 * Removes multiple cards from the deck and return an array. It is illegal
	 * to call this method if there are no more cards in the deck. You can check
	 * the number of cards remaining by calling the cardsLeft() function.
	 * 
	 * @return the set of cards which are removed from the deck.
	 * @throws IllegalStateException
	 *             if there are no cards left in the deck
	 */
	public Card[] dealMultipleCards(int noOfCardsPlayers) {
		Card[] multiCardArr = new Card[noOfCardsPlayers];
		if (cardsUsed == deck.length)
			throw new IllegalStateException("No cards are left in the deck.");
		for (int i = 0; i < noOfCardsPlayers; i++) {
			multiCardArr[i] = deck[cardsUsed - 1];
			cardsUsed++;
		}
		return multiCardArr;
	}
